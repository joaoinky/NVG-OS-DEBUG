...
O nome completo chega ao `useradd` como argumento literal, preservando apóstrofos e aspas. O hostname aceita só os caracteres de um rótulo válido, e dados incompatíveis com os registros de conta são recusados antes da execução, inclusive os que vêm de um perfil. As senhas vão pela entrada padrão do `chpasswd`: não são interpoladas em Bash nem gravadas em `nvg-secret.sh`.

Validação: 67 testes do `nvg-installer` aprovados em 11/09/2026 na árvore reconciliada, incluindo nome literal no Bash e senhas com caracteres especiais.

O texto abaixo registra o problema original; as linhas citadas são as do commit auditado.

## Walkthrough

1. Na tela de conta, o usuário informa nome completo, hostname e senha. A validação exige hostname não vazio e senha com tamanho mínimo e confirmação; não restringe os caracteres relevantes para o shell. O nome completo não recebe essa validação. Entrada: [rust/nvg-installer/src/app.rs:1002](../../../rust/nvg-installer/src/app.rs:1002).
...
- [rust/nvg-installer/src/perfil.rs:104](../../../rust/nvg-installer/src/perfil.rs:104) também preenche hostname e nome completo a partir de um perfil; esse é outro caminho de entrada, sujeito aos controles de confiança do perfil.
