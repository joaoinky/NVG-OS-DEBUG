...
1. Em uma instalação com Btrfs recém-formatado, `mount()` cria os subvolumes `@`, `@home` e outros: [rust/nvg-installer/src/install.rs:806](../neovanguard-os-dev/rust/nvg-installer/src/install.rs:806).
...
3. Ao escolher systemd-boot em UEFI, `bootloader()` constrói `root=UUID=...`, ou a raiz do mapeador LUKS, mas não inclui a seleção do subvolume: [rust/nvg-installer/src/install.rs:1186](../neovanguard-os-dev/rust/nvg-installer/src/install.rs:1186).
4. A entrada gravada usa exatamente essa linha: [rust/nvg-installer/src/install.rs:1256](../neovanguard-os-dev/rust/nvg-installer/src/install.rs:1256). Não há alteração do subvolume padrão no fluxo examinado.
